...
2. 先刪除剛才失敗流程中已建立、且不需要保留資料的 dedicated-throughput collections，釋放已佔用的 400 RU/s 配額。
3. 在資料庫層級建立/更新 shared throughput，讓所有 collections 共用免費額度內的 1000 RU/s：
...
4. 確認 MONGODB_URI 或 MONGODB_DB_NAME 指向同一個 shared-throughput database 後，再重新執行 pnpm db:prepare。
