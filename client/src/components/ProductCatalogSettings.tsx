...
export function ProductCatalogSettings() {
    ...
    return <div className="space-y-6">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
            ...
            <section><div className="mb-2 flex items-center justify-between"><h4 className="font-semibold">第三階</h4><span className="text-xs text-muted-foreground">各階末端產品皆可供商機選擇</span></div><div className="space-y-2">{items.filter(item => item.level === 3).map(item => <div key={item.id}>{renderItem(item)}<p className="mt-1 flex items-center gap-1 pl-2 text-[11px] text-muted-foreground"><ChevronRight className="h-3 w-3" />{items.find(parent => parent.id === item.parentId)?.name || "舊資料／未分類"}</p></div>)}</div></section>
        </div>
