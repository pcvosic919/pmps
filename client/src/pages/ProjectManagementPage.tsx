...
export function ProjectManagementPage() {
    ...
    const filtered = srs?.filter((sr: any) => {
        const matchSearch = sr.title.toLowerCase().includes(search.toLowerCase()) ||
            String(sr.id).includes(search);
        const matchStatus = filterStatus === "all" || sr.status === filterStatus;
        return matchSearch && matchStatus;
    }) ?? [];

    const summary = {
        total: srs?.length ?? 0,
        new: srs?.filter((s: any) => s.status === "new").length ?? 0,
        inProgress: srs?.filter((s: any) => s.status === "in_progress").length ?? 0,
        completed: srs?.filter((s: any) => s.status === "completed").length ?? 0,
    };
